...
def select_nasabah(no_rekening) :
    try :
        cur.execute("SELECT no_rekening,  nasabah.id_nasabah ,nama_nasabah, saldo, nama_cabang FROM nasabah, rekening, cabang_bank WHERE nasabah.id_nasabah=rekening.id_nasabah AND cabang_bank.kode_cabang=rekening.kode_cabang AND rekening.no_rekening LIKE ?", [no_rekening])
            ...
            for x in cur.fetchall() :
                ...
                if no_rekening in x[0] :
                    ...
                    if menu == '1' :
                        ...
                        cur.execute("UPDATE rekening SET saldo = ? WHERE no_rekening LIKE ?", [hasil, no_rekening])
                        connect.commit()
